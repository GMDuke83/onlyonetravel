...
function fixture(){
  const env={DB:database(),ALLOW_LEGACY_STAFF:'true',ZIRAAT_VERIFIED:'true',STAFF_LOGIN_KEY:'test-staff-key-'.repeat(4),ZIRAAT_CLIENT_ID:'merchant-test',ZIRAAT_STORE_KEY:'bank-test-key',SITE_URL:'https://travel.test'};
...
test('server validates refunds, mixed currencies, immutable ledger and pending charge locks',async()=>{
 const f=fixture(),guest=await f.login(),staff=await f.login(true);let r=await offered(f,guest,staff);let accepted=await f.get(r.id,guest);accepted.status='accepted';await f.put(accepted,guest);r=await f.get(r.id,staff);r.status='payopen';
 r.folio.payments.push({id:'deposit123',reference:'test-deposit',type:'deposit',method:'ziraat',amount:200,currency:'USD',exchangeRate:0.9,status:'pending'});r.payment={requestId:'deposit123'};
 ...
 r.folio.payments.push({id:'cash123',reference:'test-cash',type:'balance',method:'cash',amount:820,currency:'EUR',status:'paid'});assert.equal((await f.put(r,staff)).status,409);
 ...
 r.folio.payments.push({id:'refund123',reference:'test-refund',type:'refund',method:'cash',amount:181,currency:'EUR',status:'paid'});assert.equal((await f.put(r,staff)).status,409);
...
test('exchange rates target the offer currency rather than a hardcoded EUR base',async()=>{
 const f=fixture(),guest=await f.login(),staff=await f.login(true);let r=await f.create(guest);r=await f.get(r.id,staff);r.offer={price:1000,currency:'TRY'};r.status='offer';r=(await f.put(r,staff)).data.request;let a=await f.get(r.id,guest);a.status='accepted';await f.put(a,guest);r=await f.get(r.id,staff);
 r.folio.payments.push({id:'foreign123',reference:'test-foreign',type:'deposit',method:'cash',amount:10,currency:'USD',exchangeRate:30,baseCurrency:'EUR',status:'paid'});
...
test('individual staff roles enforce field privacy and revocation across devices',async()=>{
 const f=fixture(),owner=await f.login(true),guest=await f.login();const offeredRequest=await offered(f,guest,owner);
 const make=async(role)=>{const u=await f.call('users','POST',{name:role,role},owner);assert.equal(u.status,201);const login=await f.call('session','POST',{staffKey:u.data.token});return {cookie:login.cookie,id:u.data.id};};
 const ops=await make('operations'),sales=await make('sales'),finance=await make('finance'),reader=await make('readonly');
 const r=await f.get(offeredRequest.id,ops.cookie);assert.equal(r.offer.internalNote,undefined);assert.equal(r.staffNote,undefined);
 assert.equal((await f.call('services','GET',null,ops.cookie)).status,403);
 assert.equal((await f.call('requests/'+r.id+'/events','GET',null,ops.cookie)).status,403);
 assert.equal((await f.call('users','POST',{name:'No',role:'admin'},sales.cookie)).status,403);
 assert.equal((await f.put({...r,status:'confirmed'},ops.cookie)).status,403);
 assert.equal((await f.call('requests/'+r.id+'/claim','POST',{},reader.cookie)).status,403);
 assert.equal((await f.call('requests/'+r.id+'/claim','POST',{},sales.cookie)).status,200);
 let a=await f.get(r.id,finance.cookie);a.offer.price=1;assert.equal((await f.put(a,finance.cookie)).status,403);
 await f.call('users/'+ops.id,'PUT',{role:'operations',active:false},owner);
 assert.equal((await f.call('operations','GET',null,ops.cookie)).status,401);
 assert.equal((await f.call('audit','GET',null,owner)).data.events.some(e=>e.action==='user-access-changed'),true);
});
test('quote versions are immutable and stale acceptance cannot accept a revised price',async()=>{
 const f=fixture(),owner=await f.login(true),guest=await f.login();let r=await offered(f,guest,owner);const old=await f.get(r.id,guest);
 r.offer.price=1100;r=(await f.put(r,owner)).data.request;
 const versions=(await f.call('requests/'+r.id+'/quote-versions','GET',null,guest)).data.versions;
 assert.deepEqual(versions.map(v=>v.data.price),[1000,1100]);assert.equal(versions[0].data.internalNote,undefined);
 old.status='accepted';assert.equal((await f.put(old,guest)).status,409);
 assert.throws(()=>f.env.DB.raw.prepare('UPDATE quote_versions SET data=?').run('{}'),/immutable-quote/);
 const current=await f.get(r.id,guest);current.status='accepted';assert.equal((await f.put(current,guest)).status,200);
 r=await f.get(r.id,owner);assert.equal((await f.put({...r,status:'offer'},owner)).status,409);r.offer.price=1200;assert.equal((await f.put(r,owner)).status,409);
});
test('confirmation requires payment and atomically produces booking, agenda and tasks once',async()=>{
 const f=fixture(),owner=await f.login(true),guest=await f.login();let r=await offered(f,guest,owner);
 let a=await f.get(r.id,guest);a.status='accepted';await f.put(a,guest);r=await f.get(r.id,owner);
 assert.equal((await f.put({...r,status:'confirmed'},owner)).status,409);
 r.folio.payments.push({id:'bank-proof-123',reference:'statement-123',type:'balance',method:'partner',amount:1000,currency:'EUR',status:'paid',instructions:'Pay supplier'});
 r=(await f.put(r,owner)).data.request;r.status='confirmed';const confirmed=await f.put(r,owner);assert.equal(confirmed.status,200,JSON.stringify(confirmed.data));
 assert.equal((await f.put(r,owner)).status,409);
 assert.equal((await f.call('calendar','GET',null,owner)).data.events.length,1);
 assert.equal((await f.call('tasks','GET',null,owner)).data.tasks.length,2);
 assert.equal((await f.call('requests/'+r.id+'/booking','GET',null,guest)).data.booking.quote_version,1);
 assert.equal(f.env.DB.raw.prepare('SELECT count(*) n FROM bookings').get().n,1);
});
test('database rejects overlapping resources and allows adjacent intervals',async()=>{
 const f=fixture(),guest=await f.login();await f.create(guest);
 const add=f.env.DB.raw.prepare('INSERT INTO calendar_events VALUES(?,?,?,?,?,?,?,1,?,?)');
 add.run('one',f.input.id,'Transfer','2026-10-01T10:00:00.000Z','2026-10-01T11:00:00.000Z','Europe/Istanbul','car-1',Date.now(),'test');
 assert.throws(()=>add.run('two',f.input.id,'Transfer','2026-10-01T10:30:00.000Z','2026-10-01T11:30:00.000Z','Europe/Istanbul','car-1',Date.now(),'test'),/resource-conflict/);
 add.run('three',f.input.id,'Transfer','2026-10-01T11:00:00.000Z','2026-10-01T12:00:00.000Z','Europe/Istanbul','car-1',Date.now(),'test');
});
test('bank gateway remains closed until merchant integration is verified',async()=>{
 const f=fixture();f.env.ZIRAAT_VERIFIED='false';const response=await f.bankStart({id:f.input.id},await f.login());assert.equal(response.status,503);
});
