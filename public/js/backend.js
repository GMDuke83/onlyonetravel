...
(function(global){
  ...
  global.createOnlyoneBackend=function({getState,render,notify}){
    ...
    async function flush(){
      ...
      try{
        while(pending.size){
          ...
          baseline.set(id,clone(result.request));
          if(pending.get(id)!==job){pending.get(id).version=result.request._version;cache();continue;}
          pending.delete(id);
    ...
    async function importLegacy(){
      ...
      const preview=legacy.map(r=>(r.code||r.id)+'  -  '+(r.contact?.first||'')+'  -  '+(r.from||'')).join('\n');
      if(!window.confirm('Importvorschau: '+legacy.length+' Anfragen\n'+preview.slice(0,3000)+'\nHistorische Zahlungen werden nicht bestaetigt. Originaldaten bleiben erhalten. Importieren?'))return;
      for(const old of legacy){
        const bytes=await crypto.subtle.digest('SHA-256',new TextEncoder().encode('onlyone-legacy:'+old.id));
