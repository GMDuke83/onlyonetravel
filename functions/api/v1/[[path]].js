...
export async function onRequest({request,env}) { return safely(async()=>{
  ...
  if(path[0]==='session'){
    ...
    if(method==='POST'){
      ...
      if(b.staffKey!==undefined){
        ...
        const user=await env.DB.prepare('SELECT id,name,role FROM users WHERE token_hash=? AND active=1').bind(hash).first();
        const legacy=localOnly(request)&&env.ALLOW_LEGACY_STAFF==='true'&&env.STAFF_LOGIN_KEY?.length>=32&&hash===await digest(env.STAFF_LOGIN_KEY);
        await env.DB.prepare('INSERT INTO audit_events(action,actor,entity_id,at) VALUES(?,?,NULL,?)').bind(user||legacy?'login-success':'login-failed',user?.id||'anonymous',Date.now()).run();
        if(!user&&!legacy)fail(401,'invalid-credentials');
        return createSession(request,env,'staff',user?.name||String(b.name||'Local administrator').slice(0,100),user);
      }
      if(s)return json({role:s.role,name:s.name,permissionRole:s.permissionRole,scope:s.token_hash.slice(0,16)});
  ...
  if(['operations','calendar','tasks','users','audit'].includes(path[0]))return operations({request,env,s,path,url});
  ...
  if(!path[1]){
    if(method==='GET'){
      ...
      return json({requests:rows.results.map(r=>publicRequest(r,visibility(s))),cursor:rows.results.length===100?rows.results.at(-1).id:null});
    }
    if(method==='POST'){
      if(s.role==='staff')requirePermission(s,'sales');
      ...
      if(existing){if(existing.owner!==s.token_hash)fail(409,'id-conflict');return json({request:publicRequest(existing,visibility(s))});}
      ...
      const number=await env.DB.prepare('INSERT INTO trip_numbers(request_id) VALUES(?) ON CONFLICT(request_id) DO UPDATE SET request_id=excluded.request_id RETURNING seq').bind(r.id).first();
      r.code='OO-'+new Date().getUTCFullYear()+'-'+String(number.seq).padStart(5,'0');
      const inserts=[env.DB.prepare('INSERT INTO requests VALUES(?,?,?,1,?,?)').bind(r.id,s.token_hash,JSON.stringify(r),Date.now(),s.role+':'+(s.user_id||s.name))];
      if(b.legacy)inserts.push(env.DB.prepare('INSERT INTO legacy_imports VALUES(?,?,?)').bind(r.id,JSON.stringify(input),Date.now()));
      await env.DB.batch(inserts);
      return json({request:publicRequest({data:r,version:1},visibility(s))},201);
    }
  }
  const {row,data}=await owned(request,env,path[1]);
  if(path[2]==='claim'&&method==='POST'){
    requirePermission(s,'sales');
    if(data.assignedTo&&data.assignedTo!==(s.user_id||s.name))fail(409,'already-assigned');
    data.assignedTo=s.user_id||s.name;
    if(data.status==='new'){data.status='review';data.history.push({s:'review',at:Date.now()});}
    const result=await env.DB.prepare('UPDATE requests SET data=?,version=version+1,updated_at=?,actor=? WHERE id=? AND version=?').bind(JSON.stringify(data),Date.now(),s.user_id||s.name,row.id,row.version).run();
    if(!result.meta.changes)fail(409,'version-conflict');return json({ok:true});
  }
  if(path[2]==='quote-versions'&&method==='GET'){
    const rows=(await env.DB.prepare('SELECT version,data,created_at FROM quote_versions WHERE request_id=? ORDER BY version').bind(row.id).all()).results;
    return json({versions:rows.map(v=>{const o=JSON.parse(v.data);if(!can(s,'costs'))delete o.internalNote;return {...v,data:o};})});
  }
  if(path[2]==='booking'&&method==='GET')return json({booking:await env.DB.prepare('SELECT id,request_id,quote_version,created_at FROM bookings WHERE request_id=?').bind(row.id).first(),events:(await env.DB.prepare('SELECT id,title,starts_at,ends_at,timezone FROM calendar_events WHERE request_id=?').bind(row.id).all()).results});
  ...
  if(method==='GET')return json({request:publicRequest(row,visibility(s))});
  if(method==='PUT'){
    const b=await bodyJSON(request);
    if(b.version!==row.version)fail(409,'version-conflict');
    if(s.role==='staff'){
      if(!can(s,'sales')&&!can(s,'finance'))fail(403,'permission-denied');
      if(JSON.stringify(b.request?.offer)!==JSON.stringify(data.offer))requirePermission(s,'sales');
      if(JSON.stringify(b.request?.folio?.payments||[])!==JSON.stringify(data.folio?.payments||[]))requirePermission(s,'finance');
      if(JSON.stringify(b.request?.payment)!==JSON.stringify(data.payment))requirePermission(s,'finance');
      if(['confirmed','offer','review'].includes(b.request?.status)&&b.request.status!==data.status)requirePermission(s,'sales');
    }
    ...
    const result=await env.DB.prepare('UPDATE requests SET data=?,version=version+1,updated_at=?,actor=? WHERE id=? AND version=? AND (?=0 OR NOT EXISTS(SELECT 1 FROM payment_attempts WHERE request_id=requests.id AND status=\'pending\'))').bind(JSON.stringify(r),Date.now(),s.role+':'+(s.user_id||s.name),row.id,row.version,financial?1:0).run();
    if(!result.meta.changes)fail(409,'version-conflict-or-payment-in-progress');
    return json({request:publicRequest({data:r,version:row.version+1},visibility(s))});
  }
