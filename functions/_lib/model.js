...
export function publicRequest(row,role) {
  ...
  if(role!=='staff'){
    delete r.sourcing;delete r.assignedTo;
    delete r.staffNote;if(r.offer)delete r.offer.internalNote;
    if(r.folio)r.folio.payments=r.folio.payments.map(p=>Object.fromEntries(['id','type','method','amount','currency','exchangeRate','baseAmount','status','createdAt','paidAt','reference','instructions','due'].filter(k=>p[k]!==undefined).map(k=>[k,p[k]])));
...
export function updateRequest(old,input,role,name) {
  ...
  if(staff){
    ...
    if(input.offer && JSON.stringify(input.offer)!==JSON.stringify(old.offer)){
      ...
      if(['accepted','payopen','paid','confirmed'].includes(old.status))fail(409,'accepted-offer-immutable');
      const o=input.offer;
      if(o.validUntil&&(!/^\d{4}-\d{2}-\d{2}$/.test(o.validUntil)||!Number.isFinite(Date.parse(o.validUntil))||new Date(o.validUntil).toISOString().slice(0,10)!==o.validUntil||o.validUntil<new Date().toISOString().slice(0,10)))fail(400,'invalid-offer-expiry');
    ...
    if(incoming.length>previous.length){
      if(!r.offer)fail(409,'offer-required');if(!['accepted','payopen','paid','confirmed'].includes(old.status))fail(409,'accepted-offer-required');const p=incoming.at(-1);
      if(!str(p.reference).trim())fail(400,'payment-reference-required');
      if(p.due&&(!/^\d{4}-\d{2}-\d{2}$/.test(p.due)||!Number.isFinite(Date.parse(p.due))||new Date(p.due).toISOString().slice(0,10)!==p.due))fail(400,'invalid-payment-due');
      ...
      r.folio.payments.push({id:p.id,type:p.type,method:str(p.method,40),amount,currency:cur,exchangeRate:rate,baseAmount:base/100,baseCurrency:r.offer.currency,reference:str(p.reference,200),notes:str(p.notes),instructions:str(p.instructions),due:str(p.due,10),status:p.status,createdAt:Date.now(),...(p.status==='paid'?{paidAt:Date.now()}:{}),staff:name});
    }
    if(input.payment && r.offer){
      if(!['accepted','payopen','paid','confirmed'].includes(old.status))fail(409,'accepted-offer-required');
  ...
  const transitions={new:['review','offer'],review:['offer'],offer:[],accepted:['payopen','paid','confirmed'],payopen:['paid','confirmed'],paid:['payopen','confirmed'],confirmed:[]};
  ...
  if(next==='confirmed'&&(!r.from||totals(r).due>0))fail(409,'paid-dated-trip-required');
