...
export async function session(request,env) {
  ...
  for(const key of ['oo_staff','oo_session']){
    ...
    if(!s)continue;
    if(s.role==='staff'&&(!env.STAFF_LOGIN_KEY||s.credential_hash!==await digest(env.STAFF_LOGIN_KEY)))continue;
    return s;
  }
  return null;
}
export async function createSession(request,env,role='guest',name='Guest') {
  const raw=token(),hash=await digest(raw),ttl=role==='staff'?28800:2592000;
  await env.DB.prepare('INSERT INTO sessions VALUES(?,?,?,?,?)').bind(hash,role,name,Date.now()+ttl*1000,role==='staff'?await digest(env.STAFF_LOGIN_KEY):null).run();
  ...
  const r=json({role,name,scope:hash.slice(0,16)});r.headers.set('Set-Cookie',cookie);return r;
}
...
export async function safely(fn) { try{return await fn();}catch(e){ if(!e.status) console.error('Backend failure',e.message);return json({error:e.status?e.message:'server-error'},e.status||500); } }
